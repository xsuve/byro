...
Process.updateProcess = (id, data, result) => {
  sql.query(
    ...
    [data.userId, data.slug, data.icon, data.title, data.description, data.official, id],
    (err, res) => {
      if(err) {
        console.log('[Process] '.red + 'Error: ', err);
        result(null, err);
...
Process.deleteDraftFieldsValues = (id, result) => {
  sql.query('DELETE FROM process_fields_values WHERE processDraftId = ?', id, (err, res) => {
    if(err) {
      console.log('[Process] '.red + 'Error: ', err);
      result(null, err);
      return;
...
Process.getProcessFields = (id, result) => {
  sql.query(`SELECT * FROM process_fields WHERE processId = ${id}`, (err, res) => {
    if(err) {
      console.log('[Process] '.red + 'Error: ', err);
      result(err, null);
      return;
...
Process.getProcessDocuments = (id, result) => {
  sql.query(`SELECT PD.*, DT.slug, DT.title FROM process_documents PD, document_types DT WHERE PD.documentTypeId = DT.id AND PD.processId = ${id}`, (err, res) => {
    if(err) {
      console.log('[Process] '.red + 'Error: ', err);
      result(err, null);
      return;
...
Process.userHasProcessDocument = (data, result) => {
  sql.query(`SELECT * FROM documents D, process_documents PD WHERE D.documentTypeId = PD.documentTypeId AND D.documentTypeId = ${data.documentTypeId} AND D.userId = ${data.userId}`, (err, res) => {
    if (err) {
      console.log('[Process] '.red + 'Error: ', err);
      result(err, null);
      return;
...
Process.completeProcess = async (data, result) => {
  const templateFile = path.resolve(__dirname, '../templates/' + data.processId + '.html');
...
Process.getDraftsByUserId = (id, result) => {
  sql.query(`SELECT DR.*, PR.userId, PR.slug, PR.icon, PR.title, PR.description, PR.official FROM process_drafts DR, processes PR WHERE DR.processId = PR.id AND DR.userId = ${id}`, (err, res) => {
    if(err) {
      console.log('[Process] '.red + 'Error: ', err);
      result(err, null);
      return;
...
Process.getDraftFieldsValuesByDraftId = (id, result) => {
  sql.query(`SELECT * FROM process_fields PF, process_fields_values PFV WHERE PF.id = PFV.processFieldId AND PFV.processDraftId = ${id}`, (err, res) => {
    if(err) {
      console.log('[Process] '.red + 'Error: ', err);
      result(err, null);
      return;
