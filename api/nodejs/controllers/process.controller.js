  ...
  Process.deleteDraftFieldsValues(req.params.id, (err, data) => {
    if(err) {
      if(err.kind === 'not_found') {
        ...
        res.status(500).send({
          message: 'Could not delete fields values for Draft with id ' + req.params.id
        });
      }
    } else {
  ...
  Process.getProcessDocuments(req.params.id, (err, data) => {
    if(err) {
      if(err.kind === 'not_found') {
        ...
        res.status(500).send({
          message: 'Error retrieving Process documents with id ' + req.params.id
        });
      }
    } else {
      res.send(data);
...
exports.completeProcess = (req, res) => {
  Process.completeProcess(req.body, (err, data) => {
    if(err) {
      res.status(500).send({
        message: err.message || 'Some error occurred while completing the Process.'
      });
    } else {
      res.send(data);
    }
...
exports.draftProcess = (req, res) => {
  Process.draftProcess(req.body, (err, data) => {
    if(err) {
      res.status(500).send({
        message: err.message || 'Some error occurred while drafting the Process.'
      });
    } else {
      res.send(data);
    }
...
exports.createProcessFieldValue = (req, res) => {
  Process.createProcessFieldValue({
    ...
    if(err) {
      res.status(500).send({
        message: err.message || 'Some error occurred while creating the Process field value.'
      });
    } else {
      res.send(data);
    }
...
exports.updateProcessFieldValue = (req, res) => {
  Process.updateProcessFieldValue(req.params.id, {
    ...
    if(err) {
      if(err.kind === 'not_found') {
        ...
        res.status(500).send({
          message: 'Error updating Process field value with id ' + req.params.id
        });
      }
    } else {
      res.send(data);
...
exports.getDraftsByUserId = (req, res) => {
  Process.getDraftsByUserId(req.params.id, (err, data) => {
    if(err) {
      res.status(500).send({
        message: err.message || 'Some error occurred while retrieving user drafts.'
      });
    } else {
      res.send(data);
    }
  ...
  Process.getDraftFieldsValuesByDraftId(req.params.id, (err, data) => {
    if(err) {
      if(err.kind === 'not_found') {
        ...
        res.status(500).send({
          message: 'Error retrieving Process fields with values with draft id ' + req.query.id
        });
      }
    } else {
      res.send(data);
