...
const logoIconWrapperVariants = cva([], {
  variants: {
    size: {
      lg: 'w-14 h-14',
      md: 'w-10 h-10',
...
const logoIconTextWrapperVariants = cva([], {
  variants: {
    size: {
      lg: 'w-32',
      md: 'w-28',
...
const Comp: React.FC<LogoProps> = ({
  ...
  return (
    <div className={cn(className)}>
      {variant === 'icon' && (
        <svg
          ...
          className={cn(logoIconWrapperVariants({ size }))}>
          <path
            className={logoIconVariants({ color })}
	...
	c7.2-6.3,18-6,24.9,0.9c7,7,7.1,18.4,0.2,25.6l-0.1,0.1l-2.8,2.8L88.2,52.4z'
      ...
      {variant === 'icon-text' && (
        <svg
          viewBox='0 0 319.5 112.2'
          className={cn(logoIconTextWrapperVariants({ size }))}>
          <path
            className={cn(logoIconVariants({ color }))}
            d='M106.8,65.5l0.1-0.2c10.5-10.7,10.5-27.9-0.1-38.5c-7.4-7.4-18.1-9.7-27.5-6.8c-0.1-5.1-2.1-10.2-6-14
	...
	l0,0l-3.4-3.3l3-3l0.5-0.5l0.2-0.2c7.2-6.3,18-6,24.9,0.9c7,7,7.1,18.4,0.2,25.6l-0.1,0.1L97.7,62L87,51.3z'
          ...
          <g>
            <path
              className={cn(logoTextVariants({ color }))}
              d='M178.3,81.8c-3,0-5.5-0.5-7.6-1.5c-2-1-3.8-2.3-5.4-4l0.9-2.5v7h-11.5V16.1h11.4v35v-3.7c1.4-1.2,3.3-2.2,5.7-3
		...
		C172.9,71.9,174.2,72.1,175.7,72.1z'
            ...
            <path
              className={cn(logoTextVariants({ color }))}
              d='M218.4,81l-17.5-36h12.9l13.5,30.3L218.4,81z M208.8,98L232.6,45H243L219.8,98H208.8z'
            />
            <path
              className={cn(logoTextVariants({ color }))}
              d='M260,45l1.1,9.8l-0.2-1.5c1-2.1,2.5-3.9,4.3-5.5c1.8-1.5,3.6-2.7,5.4-3.5c1.8-0.8,3.1-1.2,4.1-1.2l-0.6,11.5
		c-2.7-0.3-5,0.1-6.9,1.3c-1.9,1.2-3.4,2.8-4.4,4.8c-1,2-1.6,4-1.6,6.1v14h-11.4V45H260z'
            />
            <path
              className={cn(logoTextVariants({ color }))}
              d='M278.9,62.4c0-3.7,0.9-6.9,2.6-9.8c1.7-2.9,4.1-5.2,7.3-6.8c3.1-1.7,6.7-2.5,10.7-2.5c4.1,0,7.7,0.8,10.7,2.5
